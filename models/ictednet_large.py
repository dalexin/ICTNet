...
class ICTEDNet(nn.Module):
    ...
    def __init__(self, backbone_name='resnet18', class_num=19, augment=True):
        ...
        self.ms_context = DAPPM(512, 96, 128)
        self.ml_context1 = MLContext(low_channels = 128, high_channels = 256, out_channels = 256)
        self.ml_context2 = MLContext(low_channels = 256, high_channels = 128, out_channels = 256)
    ...
    def forward(self, x):
        ...
        feat_ml_context1 = self.ml_context1(feat_ms_context, feat3)
        feat_ml_context2 = self.ml_context2(feat_ml_context1, feat2)

        feature_ic_refine = self.icpg(feat_ml_context2, ic_feature)
...
if __name__ == '__main__':
    ...
    model = get_pred_model(backbone_name='resnet18', num_classes=11)
    model.eval()
    model.to(device)
    iterations = None
